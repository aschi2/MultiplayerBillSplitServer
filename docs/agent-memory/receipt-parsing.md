...
- Standard parse uses `thinkingBudget=-1` (dynamic — model decides per-request) and `max_output_tokens=6800`. Dynamic thinking lets flash-lite handle ~95% of receipts without escalation: simple receipts get minimal thinking, harder ones get more. flash-lite uses `thinkingBudget` (token count, where 0 disables, -1 is dynamic, valid range 512–24576), NOT `thinkingLevel` — that's only on 3.x pro models.
