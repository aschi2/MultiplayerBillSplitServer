...
- Standard parse uses `thinkingBudget=4096` (fixed) and `max_output_tokens=6800`. Fixed budget gives flash-lite consistent reasoning headroom on every receipt; dynamic budget (`-1`) was inconsistent and missed items on receipts the model deemed "simple". flash-lite uses `thinkingBudget` (token count, where 0 disables, -1 is dynamic, valid range 512–24576), NOT `thinkingLevel` — that's only on 3.x pro models.
- Retry/high-accuracy parse uses `thinkingLevel=MEDIUM` and `max_output_tokens=24000`, with NO `response_schema`. The schema's `nullable: true` price fields combined with MEDIUM thinking caused the pro model to emit null prices when uncertain (decoding to $0.00 in UI). Free-form JSON + the detailed retry prompt + `cleanModelJSON` is reliable and preserves price extraction. Prompt explicitly tells the model to always populate prices when visible.
